...
(ert-deftest grove-inbox-review-excludes-daily-notes ()
  (let* ((grove-directory (file-name-as-directory (make-temp-file "grove-vault" t)))
         (daily (expand-file-name "daily" grove-directory))
         (daily-file (expand-file-name "2026-06-25.org" daily))
         (root-file (expand-file-name "root.org" grove-directory)))
    (unwind-protect
        (let ((grove--cache (make-hash-table :test #'equal))
              (grove-daily-directory "daily")
              (grove-inbox-review-excluded-directories '("daily")))
          (make-directory daily)
          (with-temp-file daily-file (insert "#+title: 2026-06-25\n"))
          (with-temp-file root-file (insert "#+title: Root\n"))
          (puthash daily-file (grove--parse-note daily-file) grove--cache)
          (puthash root-file (grove--parse-note root-file) grove--cache)
          (should (equal (grove-inbox--untagged-notes)
                         `(("Root" . ,root-file)))))
      (delete-directory grove-directory t))))
