...
| =C-c v T= | =grove-add-filetag=   | Add file tags to note    |
...
=C-c v T= runs =grove-add-filetag= in a Grove note. It prompts for one or more comma-separated file tags with completion from tags already present in the vault, then creates or updates the note's =#+filetags:= line.

** Inbox review

=C-c v i= opens =*grove-inbox*=, a triage buffer that lists notes needing attention. The buffer groups notes by the enabled =grove-inbox-review-checks=: =untagged=, =no-backlinks=, and/or =in-inbox=. Files under =grove-inbox-review-excluded-directories= are skipped; by default this excludes daily notes.
...
| =R=         | Refile note to a directory   |
| =T=         | Add file tags to note        |
| =D=         | Delete the note at point     |
| =q=         | Close the buffer             |

The intended workflow is short loops: open the inbox, jump into a note with =RET=, add tags or wikilinks, save, and either come back with =C-c v i= or refresh in place with =g=. Use =R= to refile the note out of =grove-inbox-directory=; the default destination is the vault root, one directory up from the default inbox. Notes drop off the list as you tag them, so the buffer doubles as a progress indicator.
